...
public class CardActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            showCamera = new ShowCamera(CardActivity.this, cameraView);
                                            ...
                                            if (responseFile.body().status == 1) {
                                                ...
                                                try {
                                                    if (userItem.userType.equals(UserTypeContant.STUDENT)) {
                                                        //Học viên đăng nhập
                                                        dialog.dismiss();
                                                        //kiem tra xem hoc vien da dong bo khuon mat len he thong chua
...
//                                                                                intentMenu = new Intent(CardActivity.this, AddFaceActivity2.class);
                                                                            } else {
//                                                                                intentMenu = new Intent(CardActivity.this, AddFaceActivity.class);
                                                                            }
//                                                                            startActivity(intentMenu);
                                                ...
                                                Toast.makeText(CardActivity.this, responseFile.body().message, Toast.LENGTH_SHORT).show();
                                            }
                                        } else {
                                            Toast.makeText(CardActivity.this, R.string.CONNECTION_SERVER_ERROR, Toast.LENGTH_SHORT).show();
                                        }
                                ...
                                CustomToast.makeText(CardActivity.this, response.body().message, Toast.LENGTH_SHORT, 3).show();
                                if (progressDialog != null && progressDialog.isShowing()) {
                                    progressDialog.dismiss();
                                }
                            }
                        ...
                        CustomToast.makeText(CardActivity.this, "Có lỗi xảy ra", Toast.LENGTH_SHORT, 3).show();
                    }
                }
...
//            startActivity(intentLogin);
    ...
    private void StudentLogin() {
        ...
        UserItem teacherItem = SharedPreferencesUtil.getTeacher(CardActivity.this);
        if (teacherItem != null) {
                    ...
                    if (response.body().status == 1) {
                        ...
                        if (inOutModel.Type == 1) {
                            if (response.body().getSessionId() != null) {
                                SharedPreferencesUtil.setUser(CardActivity.this, userItem);
                                SharedPreferencesUtil.setSession(CardActivity.this, response.body().getSessionId());
                                CustomToast.makeText(CardActivity.this, "Đăng nhập học viên thành công!", 1000, 1).show();
                            } else {
                                CustomToast.makeText(CardActivity.this, "Không tạo được phiên học, vui lòng thử lại!",
                                        Toast.LENGTH_SHORT, 3).show();
                                return;
                            }
                        }
                        if (SharedPreferencesUtil.checkGoToMain(CardActivity.this)) {
                            try {
                                Intent intentLogin;
                                if (CameraHelperSingleton.getInstance().getUsbDevice() != null) {
//                                    intentLogin = new Intent(CardActivity.this, DashbroadActivity2.class);
                                } else {
//                                    intentLogin = new Intent(CardActivity.this, DashbroadActivity.class);
                                }
//                                startActivity(intentLogin);
                            ...
                            try {
                                Intent intentLogin = new Intent(CardActivity.this, LoginActivity.class);
                                startActivity(intentLogin);
                                if (dialog != null && dialog.isShowing()) {
                                    dialog.dismiss();
                                }
                        ...
                        if (response.body().message != null) {
                            CustomToast.makeText(CardActivity.this, "Thông tin: " + response.body().message,
                                    Toast.LENGTH_SHORT, 3).show();
                        } else {
                            CustomToast.makeText(CardActivity.this, "Kết nối server lỗi!",
                                    Toast.LENGTH_SHORT, 3).show();
                        }
                    }

                } else {
                    CustomToast.makeText(CardActivity.this, "Lỗi kết nối", Toast.LENGTH_SHORT, 3).show();
                }
            }
    ...
    private void TeacherLogin() {
        ...
        String sessionId = SharedPreferencesUtil.getSession(CardActivity.this);
            ...
            public void onResponse(Call<TeacherLoginResponse> call, Response<TeacherLoginResponse> response) {
                if (response.body() != null) {
                    if (response.body().status == 1) {
                        ...
                        if (inOutModel.Type == 1) {
                            SharedPreferencesUtil.setUser(CardActivity.this, userItem);
                            CustomToast.makeText(CardActivity.this, "Đăng nhập giảng viên thành công!", 1000, 1).show();
                        }
                        if (SharedPreferencesUtil.checkGoToMain(CardActivity.this)) {
                            try {
                                Intent intentLogin;
...
//        final Dialog dialog = new Dialog(this, android.R.style.Theme_Black_NoTitleBar_Fullscreen);
        ...
        MediaPlayer mPlayer = MediaPlayer.create(CardActivity.this, R.raw.start);
        mPlayer.start();
            ...
            btSave.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    try {
                        ...
                        progressDialog.show();
                        UserItem student = SharedPreferencesUtil.getStudent(CardActivity.this);
                        UserItem teacher = SharedPreferencesUtil.getTeacher(CardActivity.this);
                        if (student != null && userItem != null && userItem.userType.equals(UserTypeContant.STUDENT) && student.code.equals(userItem.code)) {
                            CustomToast.makeText(CardActivity.this, "Vui lòng đăng xuất học viên trước khi đăng nhập!", 1000, 2).show();
                            if (progressDialog != null && progressDialog.isShowing()) {
                                progressDialog.dismiss();
                            }
                            return;
                        ...
                        if (teacher != null && userItem != null && userItem.userType.equals(UserTypeContant.TEACHER) && teacher.code.equals(userItem.code)) {
                            CustomToast.makeText(CardActivity.this, "Vui lòng đăng xuất giảng viên trước khi đăng nhập!", 1000, 2).show();
                            if (progressDialog != null && progressDialog.isShowing()) {
                                progressDialog.dismiss();
                            }
                            return;
                                    ...
                                    if (response.body() != null) {
                                        if (response.body().status == 1) {
                                            ...
                                            if (listVehicle == null || listVehicle.size() == 0) {
                                                CustomToast.makeText(CardActivity.this, "Bạn chưa được gán đến thiết bị này!", Toast.LENGTH_SHORT, 2).show();
                                                if (progressDialog != null && progressDialog.isShowing()) {
                                                    progressDialog.dismiss();
                                                }
                                            } else {
                        ...
                        callback.enqueue(new Callback<GetUserByIdCardResponse>() {
                            ...
                            public void onResponse(Call<GetUserByIdCardResponse> call, Response<GetUserByIdCardResponse> response) {
                                if (response.body() != null) {
                                    ...
                                    CustomToast.makeText(CardActivity.this, String.valueOf(R.string.CONNECTION_SERVER_ERROR), Toast.LENGTH_SHORT, 3).show();
                                }
                            }
