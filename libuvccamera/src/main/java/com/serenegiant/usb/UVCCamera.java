...
//            throw new UnsupportedOperationException("open failed:result=" + result);
    ...
    public synchronized void close(boolean isSilent) {
        ...
        if (mNativePtr != 0) {
            // tạm thời đóng
//            nativeRelease(mNativePtr); // It is possible that calling the NativeStopPreview method will always block, causing the camera to not open the next time.(https://github.com/jiangdongguo/AndroidUSBCamera/issues/422)
...
//        setFrameCallback(null, 0);
        if (mCtrlBlock != null) {
            try {
                // tạm thời đóng
//                nativeStopPreview(mNativePtr); // It is possible that calling the NativeStopPreview method will always block, causing the camera to not open the next time.(https://github.com/jiangdongguo/AndroidUSBCamera/issues/422)
            } catch (final Exception e) {
    ...
    public synchronized void destroy(boolean isSilent) {
        ...
        if (mNativePtr != 0) {
            // tạm thời đóng
//            nativeDestroy(mNativePtr); // It is possible that calling the NativeStopPreview method will always block, causing the camera to not open the next time.(https://github.com/jiangdongguo/AndroidUSBCamera/issues/422)
            mNativePtr = 0;
        }
    }
